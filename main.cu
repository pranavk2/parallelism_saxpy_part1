...
// return GB/s
double toBW(long bytes, float sec) {
...
int main(int argc, char** argv)
{
	...
	while ((opt = getopt_long(argc, argv, "c:n:p:i:", long_options, NULL)) != EOF) {
	...
	if (cs == 1) {
		...
		resultarray = (float*) calloc (total_elems, sizeof(float));
	}
	if (cs == 2) {
		cudaMallocManaged(&xarray, total_elems*sizeof(float));
		cudaMallocManaged(&yarray, total_elems*sizeof(float));
		cudaMallocManaged(&resultarray, total_elems*sizeof(float));
	}
	if (cs == 3) {
		cudaMallocHost(&xarray, total_elems*sizeof(float));
		cudaMallocHost(&yarray, total_elems*sizeof(float));
		cudaMallocHost(&resultarray, total_elems*sizeof(float));
	}
	//
	...
	printf("Number of bytes of one array: %ld\n", sizeof(float) * total_elems);
	printf("Overall time : %8.3lf ms [%8.3lf GB/s ]\n", 1000.f * totalTimeAvg, toBW(totalBytes, totalTimeAvg));
	printf("GPU Kernel   : %8.3lf ms [%8.3lf Ops/s]\n", 1000.f * timeKernelAvg, toBW(totalBytes/3, timeKernelAvg));
	printf("Copy CPU->GPU: %8.3lf ms [%8.3lf GB/s ]\n", 1000.f * timeCopyH2DAvg, toBW(totalBytes/3*2, timeCopyH2DAvg));
